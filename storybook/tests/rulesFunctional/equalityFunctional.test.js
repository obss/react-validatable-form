...
test('equality functional equalTo test', () => {
    const initialFormData = {
        val: 'same value',
        funcValue: 'same value',
    };
    const rules = [
        {
            path: 'val',
            ruleSet: [{ rule: 'required' }, { rule: 'equality', notEqualTo: (formData) => formData['funcValue'] }],
            dependantPaths: ['funcValue'],
        },
    ];
    const result = testBuilder(rules, initialFormData);
...
test('equality functional isOneOf test', () => {
    ...
    const rules = [
        {
            path: 'val',
            ruleSet: [{ rule: 'required' }, { rule: 'equality', isNoneOf: (formData) => formData['funcValue'] }],
            dependantPaths: ['funcValue'],
        },
    ];
    const result = testBuilder(rules, initialFormData);
